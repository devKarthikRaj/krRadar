% Standalone Wiener Filter Test on DVB-T2 signal
...
refFile = 'C:\Users\kraj4\Documents\NTU Stuff\FYP\MATLAB krRadar DSP SITL Test Code\Recordings\538mhz_10s_ref.wav'; % Change this to your actual file path
surFile = 'C:\Users\kraj4\Documents\NTU Stuff\FYP\MATLAB krRadar DSP SITL Test Code\Recordings\538mhz_10s_sur.wav'; % Change this to your actual file path

% Read the WAV files
disp('INFO: Loading WAV file...');
...
%% Remove Reference DPI from Surveillance Using Wiener Filtering
disp('Applying Wiener Filtering to Remove DPI and Preserve Targets...');

% Compute Wiener Filter Coefficients
H_wiener = (abs(fft(IQ_complex_ref, NFFT)).^2) ./ ...
           (abs(fft(IQ_complex_ref, NFFT)).^2 + 0.2 * abs(fft(IQ_complex_sur, NFFT)).^2);
...
title('Surveillance Signal Spectrum (After Wiener Filtering)'); grid on;
xlim([min(freqAxis_ref_sur) max(freqAxis_ref_sur)]); ylim([-100, 5]);

disp('Wiener Filtering-Based Interference Cancellation Complete');